...
class ParseToken:
    def __init__(self, pos: int, rule: lex.TokenRule, length: int):
        self.pos = pos
        self.rule = rule
        self.length = length
    # returns True on EOF
    def skipInlineWhitespace(self, src: str) -> bool:
        if self.rule.kind == lex.TokenKind.INLINE_WHITESPACE:
            token_start = self.pos + self.length
            lex_token = lex.scan(src, token_start)
            if not lex_token:
                return True
            self.rule, self.length = lex_token
            assert(self.length > 0)
            # should be impossible to get 2 inline whitespace tokens in a row
            assert(self.rule.kind != lex.TokenKind.INLINE_WHITESPACE)
...
def parseNode(src: str, token: ParseToken) -> Tuple[Node, Optional[ParseToken]]:
    ...
    while True:
        ...
        elif token.rule.kind == lex.TokenKind.DELIMITED_STRING:
            node = combineNodes(node, NodeToken(token_src, token_src[3:-1]))
            next_token_start = token_end
        elif token.rule.kind == lex.TokenKind.ESCAPE_SEQUENCE:
            node = combineNodes(node, NodeToken(token_src, token_src[1:]))
            next_token_start = token_end
        else:
            raise Exception("codebug: unhandled token kind {}".format(token.rule.kind))

        lex_token = lex.scan(src, next_token_start)
        if not lex_token:
            return node, None
        rule, token_len = lex_token
        assert(token_len > 0)
...
def parseCommand(src, cmd_start) -> Tuple[List[Node],int]:
    ...
    first_rule, first_token_len = lex_token
    assert(first_token_len > 0)
    token = ParseToken(cmd_start, first_rule, first_token_len)
    ...
    while True:
        ...
        token = next_token
        if token.rule.kind == lex.TokenKind.INLINE_WHITESPACE:
            if token.skipInlineWhitespace(src):
                return nodes, token.pos
        else:
